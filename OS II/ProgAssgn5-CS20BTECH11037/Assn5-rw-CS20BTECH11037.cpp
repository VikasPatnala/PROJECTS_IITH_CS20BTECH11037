...
FILE *out_file = fopen("RW-log.txt", "w");
...
int main()
{
    ...
    // initiallizing the random variables
    ...
    avg_out << "Average Waiting Time rw : " << (double)(writers_waiting_time + readers_waiting_time) / ((nw * kw + nr * kr) * THS) << " milliseconds\n";
    avg_out << "Average Waiting Time rw writers : " << (double)(writers_waiting_time) / (nw * kw * THS) << " milliseconds\n";
    avg_out << "Average Waiting Time rw readers : " << (double)(readers_waiting_time) / (nr * kr * THS) << " milliseconds\n";
...
void *writer(void *params)
{
    ...
    for (int i = 1; i <= kw; ++i)
    {
        ...
        // clearing the lock in the sense we are releasing the lock for other threads
